...
export default function MedicalStorePage() {
  ...
  const fetchStores = async () => {
    try {
      ...
      if (selectedEmirate && selectedEmirate !== 'all') params.append('emirateId', selectedEmirate);
      
      const response = await fetch(`/api/medicine/store?${params.toString()}`);
      if (!response.ok) throw new Error('Failed to fetch medical stores');
      ...
      toast({
        title: 'Error',
        description: 'Failed to load medical stores',
        variant: 'destructive',
      });
    } finally {
      setIsLoading(false);
  ...
  const handleOpenDialog = (store: MedicalStore | null = null) => {
  ...
  const handleDeleteClick = (store: MedicalStore) => {
    setSelectedStore(store);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (selectedStore) {
        ...
        response = await fetch(`/api/medicine/store?id=${selectedStore.id}`, {
          method: 'PUT',
        ...
        // Create new store
        response = await fetch('/api/medicine/store', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify(payload),
        });
      ...
      if (!response.ok) {
        ...
        throw new Error(errorData.error || 'Failed to save medical store');
      }

      toast({
        title: 'Success',
        description: selectedStore 
          ? 'Medical store updated successfully' 
          : 'Medical store created successfully',
      });
      ...
      toast({
        title: 'Error',
        description: error.message || 'Failed to save medical store',
        variant: 'destructive',
      });
    }
  };
  ...
  const handleDelete = async () => {
    ...
    try {
      const response = await fetch(`/api/medicine/store?id=${selectedStore.id}`, {
        method: 'DELETE',
      });
      ...
      if (!response.ok) {
        if (response.status === 400) {
          ...
          throw new Error(
            errorData.hasMenus
              ? 'Cannot delete store with associated menus'
              : errorData.hasOrders
      ...
      toast({
        title: 'Error',
        description: error.message || 'Failed to delete medical store',
        variant: 'destructive',
      });
    }
  };
  ...
  }, [formData.exactLocation.lat, formData.exactLocation.lng]);
      ...
      <Card>
        <CardHeader className="flex flex-row items-center justify-between">
          <div>
            ...
            <CardDescription>
              Manage your medical stores, their locations, and operating hours
            </CardDescription>
          </div>
          ...
          <form onSubmit={handleSubmit} className="flex h-full">
            ...
            <div className="w-1/3 h-full flex flex-col border-r">
              <div className="p-6 pb-0 flex-shrink-0">
                <DialogHeader>
                  <DialogTitle>{selectedStore ? 'Edit Medical Store' : 'Add New Medical Store'}</DialogTitle>
                  <DialogDescription>
                    {selectedStore
                      ? 'Update the details of the medical store'
                      : 'Add a new medical store to the system'}
                  </DialogDescription>
                </DialogHeader>
              </div>
      ...
      <Dialog open={isDeleteDialogOpen} onOpenChange={setIsDeleteDialogOpen}>
        <DialogContent className="sm:max-w-[425px]">
          <DialogHeader>
            ...
            <DialogDescription>
              Are you sure you want to delete the medical store "{selectedStore?.name}"?
