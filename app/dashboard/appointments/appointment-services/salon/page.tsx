...
export default function SalonPage() {
  const [places, setPlaces] = useState<SalonPlace[]>([]);
  const [searchTerm, setSearchTerm] = useState('');
  const [statusFilter, setStatusFilter] = useState<string>('all');
  const [isLoading, setIsLoading] = useState(true);
  const [isDialogOpen, setIsDialogOpen] = useState(false);
  const [isDeleteDialogOpen, setIsDeleteDialogOpen] = useState(false);
  const [selectedPlace, setSelectedPlace] = useState<SalonPlace | null>(null);
  ...
  const fetchPlaces = async () => {
    try {
      ...
      const response = await fetch(`/api/appointments/appointment-places/salon?${params.toString()}`);
      if (!response.ok) throw new Error('Failed to fetch salon places');
      
      const data = await response.json();
      setPlaces(data.places || []);
    } catch (error) {
      console.error('Error fetching salon places:', error);
      toast({
        title: 'Error',
        description: 'Failed to load salon places',
        variant: 'destructive',
      });
    } finally {
      setIsLoading(false);
  ...
  const handleOpenDialog = (place: SalonPlace | null = null) => {
    if (place) {
  ...
  const handleDeleteClick = (place: SalonPlace) => {
    setSelectedPlace(place);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const url = '/api/appointments/appointment-places/salon';
      ...
      if (!response.ok) {
        ...
        throw new Error(errorData.error || 'Failed to save salon place');
      }

      toast({
        title: 'Success',
        description: `Salon place ${selectedPlace ? 'updated' : 'created'} successfully`,
      });

      setIsDialogOpen(false);
      fetchPlaces();
    } catch (error: any) {
      console.error('Error saving salon place:', error);
      toast({
        title: 'Error',
        description: error.message || 'Failed to save salon place',
        variant: 'destructive',
      });
    }
  };
  ...
  const handleDelete = async () => {
    ...
    try {
      const response = await fetch(`/api/appointments/appointment-places/salon?id=${selectedPlace.id}`, {
        method: 'DELETE',
      });

      if (!response.ok) {
        const errorData = await response.json();
        throw new Error(errorData.error || 'Failed to delete salon place');
      }

      toast({
        title: 'Success',
        description: 'Salon place deleted successfully',
      });

      setIsDeleteDialogOpen(false);
      fetchPlaces();
    } catch (error: any) {
      console.error('Error deleting salon place:', error);
      toast({
        title: 'Error',
        description: error.message || 'Failed to delete salon place',
        variant: 'destructive',
      });
    }
  };
    ...
    <div className="container mx-auto p-6">
      <Card>
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-2">
              ...
              <div>
                ...
                <CardDescription>Manage salon places and their appointments</CardDescription>
              </div>
            </div>
        ...
        <CardContent>
          ...
          <div className="flex flex-col sm:flex-row gap-4 mb-6">
            <div className="flex-1">
              <div className="relative">
                ...
                <Input
                  placeholder="Search salon places..."
                  value={searchTerm}
          ...
          {/* Table */}
          {isLoading ? (
            <div className="text-center py-4">Loading salon places...</div>
          ) : (
              ...
              <div className="p-6 pb-0 flex-shrink-0">
                <DialogHeader>
                  <DialogTitle className="flex items-center">
                    ...
                    {selectedPlace ? 'Edit Salon Place' : 'Add New Salon Place'}
                  </DialogTitle>
                  <DialogDescription>
                    {selectedPlace ? 'Update the salon place details below.' : 'Fill in the details to add a new salon place.'}
                  </DialogDescription>
                </DialogHeader>
              </div>
              ...
              <div className="flex-grow overflow-y-auto p-6 pt-4" style={{ maxHeight: 'calc(95vh - 140px)' }}>
                <div className="space-y-4">
                  <div className="space-y-2">
                    <Label htmlFor="name" className="text-sm font-medium">Salon Name <span className="text-red-500">*</span></Label>
                    <Input
                      id="name"
                      value={formData.name}
                  ...
                  <div className="space-y-2">
                    <Label htmlFor="specialistName" className="text-sm font-medium">Specialist Name</Label>
                    <Input
                      id="specialistName"
                      value={formData.specialistName}
                      onChange={(e) => setFormData({ ...formData, specialistName: e.target.value })}
                      placeholder="Hair Stylist/Beautician (Optional)"
                    />
                  </div>
              ...
              <div className="p-6 border-t bg-background">
                <DialogFooter className="flex justify-end gap-2">
                  ...
                  <Button type="submit" className="bg-pink-600 hover:bg-pink-700">
                    {selectedPlace ? 'Update' : 'Create'} Salon Place
                  </Button>
                </DialogFooter>
              </div>
            </div>
      ...
      <Dialog open={isDeleteDialogOpen} onOpenChange={setIsDeleteDialogOpen}>
        <DialogContent className="sm:max-w-[425px]">
          <DialogHeader>
            <DialogTitle>Delete Salon Place</DialogTitle>
            <DialogDescription>
              Are you sure you want to delete the salon place "{selectedPlace?.name}"?
              {selectedPlace && (selectedPlace._count?.appointments ?? 0) > 0 && (
                <div className="mt-2 text-destructive flex items-center">
                  <AlertCircle className="h-4 w-4 mr-2" />
                  This salon place has {selectedPlace._count?.appointments} appointment(s) and cannot be deleted.
                </div>
              )}
            </DialogDescription>
          </DialogHeader>
